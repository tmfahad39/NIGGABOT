module.exports.config = {
  name: "video2", // <--- Changed this line from "video" to "video2"
  version: "1.0.0",
  hasPermssion: 0,
  ...
  usages: "video2 [Text]", // <--- Changed this line for consistency in usage
  cooldowns: 10,
  dependencies: {
    "@distube/ytdl-core": "",
    "simple-youtube-api": "",
  ...
  envConfig: {
    "YOUTUBE_API": "AIzaSyDEE1-zZSRVI8lTaQOVsIAQFgL-_BJKvhk" // REMINDER: Change this to your actual API key!
  }
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (urlValid) {
    ...
    try {
      ...
      var body = `There are ${link.length} results matching your search keyword:\n\n${msg}\nPlease reply(feedback) choose one of the above searches`
      return api.sendMessage({ attachment: imgthumnail, body: body }, event.threadID, (error, info) => global.client.handleReply.push({
        name: this.config.name, // The handleReply still uses 'this.config.name' which correctly points to 'video2'
        messageID: info.messageID,
        author: event.senderID,
        link
      }),
